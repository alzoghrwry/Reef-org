<template>
  <div id="app" class="min-h-screen bg-gold-50">
    <!-- Header Component -->
    <AppHeader 
      :routes="routes"
      :language="currentLanguage"
      @toggle-language="toggleLanguage"
    />

    <!-- Main Content -->
    <main>
      <router-view />
    </main>

    <!-- Footer Component -->
    <AppFooter @subscribe="handleNewsletterSubscribe" />
  </div>
</template>

<script setup>
import { ref, defineComponent } from 'vue'
import AppHeader from './components/layout/AppHeader.vue'
import AppFooter from './components/layout/AppFooter.vue'

// Icon components
const HomeIcon = defineComponent({
  name: 'HomeIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>`
})

const AboutIcon = defineComponent({
  name: 'AboutIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"/></svg>`
})

const MapIcon = defineComponent({
  name: 'MapIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 20l-5.447-2.724A1 1 0 013 16.382V5.618a1 1 0 011.447-.894L9 7m0 13l6-3m-6 3V7m6 10l4.553 2.276A1 1 0 0021 18.382V7.618a1 1 0 00-.553-.894L15 4m0 13V4m0 0L9 7"/></svg>`
})

const ProgramsIcon = defineComponent({
  name: 'ProgramsIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>`
})

const NewsIcon = defineComponent({
  name: 'NewsIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"/></svg>`
})

const GalleryIcon = defineComponent({
  name: 'GalleryIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>`
})

const ContactIcon = defineComponent({
  name: 'ContactIcon',
  template: `<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 4.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>`
})

// Routes configuration with icon components
    const routes = [
  { path: '/', name: 'الرئيسية', icon: HomeIcon },
  { path: '/about', name: 'من نحن', icon: AboutIcon, hasDropdown: true }, // جعله قائمة منسدلة
  { path: '/climate-map', name: 'الخريطة التفاعلية', icon: MapIcon },
  { path: '/projects', name: 'المشاريع', icon: ProgramsIcon },
  { path: '/news', name: 'أخبارنا', icon: NewsIcon },
  { path: '/media-gallery', name: 'المعرض الإعلامي', icon: GalleryIcon, hasDropdown: true },
  { path: '/contact', name: 'تواصل معنا', icon: ContactIcon }
]
    
const currentLanguage = ref('ar')
    
    const toggleLanguage = () => {
      currentLanguage.value = currentLanguage.value === 'ar' ? 'en' : 'ar'
}

const handleNewsletterSubscribe = (email) => {
  console.log('Subscribing to newsletter:', email)
      alert('تم الاشتراك في النشرة البريدية بنجاح! شكراً لك.')
}
</script>
